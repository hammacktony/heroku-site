...
export class BlogService {
  ...
  public all(blog: string, query: any = null): Observable<any[]> {
    if (query) {
      var url = `${ConfigService.API_URL}/api/blog/${blog}?`;
      ...
      this.http
        .get(`${ConfigService.API_URL}/api/blog/${blog}`)
        .subscribe((response: any) => {
          this._collection$.next(JSON.parse(response._body));
        });
    }
  ...
  public get(blog: string, slug: string): Observable<any[]> {
    this.http
      .get(`${ConfigService.API_URL}/api/blog/${blog}/${slug}`)
      .subscribe((response: any) => {
        this._collection$.next(JSON.parse(response._body));
      });
  ...
  public create(blog: string, payload: Post): Observable<any[]> {
    this.http
      .post(`${ConfigService.API_URL}/api/blog/${blog}`, payload)
      .subscribe((response: any) => {
        this._collection$.next(JSON.parse(response._body));
      });
  ...
  public update(blog: string, slug: string, payload: Post): Observable<any[]> {
    this.http
      .put(`${ConfigService.API_URL}/api/blog/${blog}/${slug}`, payload)
      .subscribe((response: any) => {
        this._collection$.next(JSON.parse(response._body));
      });
